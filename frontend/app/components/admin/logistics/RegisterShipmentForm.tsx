...
export function RegisterShipmentForm() {
  ...
  const [form, setForm] = useState({ trackingNumber: "", providerName: "", providerPhone: "", purchaseDate: "" });
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setForm({ trackingNumber: "", providerName: "", providerPhone: "", purchaseDate: "" });
      setPhoto(null);
      setInvoice(null);
    } catch (err: any) {
  ...
  return (
    <form onSubmit={submit} className="grid gap-3 rounded-card border border-border bg-surface p-4 sm:grid-cols-2">
      ...
      <Field label="Teléfono del proveedor">
        <input className="input" value={form.providerPhone} onChange={set("providerPhone")} placeholder="Teléfono / WeChat" />
      </Field>
